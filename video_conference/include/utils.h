...
int thread_exit_audio_send=0;
int thread_exit_audio_recv=0;
...
void  *voice_receive(void *sock)
{
    ...
    if (!(s = pa_simple_new(NULL, socket_recv->name, PA_STREAM_PLAYBACK, NULL, "playback", &ss, NULL, NULL, &error))) {
        fprintf(stderr, __FILE__": pa_simple_new() failed: %s\n", pa_strerror(error));
        if (s)
            pa_simple_free(s);
        return NULL;
    }
    while(!thread_exit_audio_recv) {
        uint8_t buf[BUFSIZE];
        ssize_t r;
        /* Read some data ... */
        if ((r = loop_read(socket_fd, buf, sizeof(buf))) <= 0) {
            if (r == 0) /* EOF */
                break;
...
void *voice_send(void *sock)
{
    ...
    if (!(s = pa_simple_new(NULL, socket_send->name, PA_STREAM_RECORD, NULL, "record", &ss, NULL, NULL, &error))) {
        fprintf(stderr, __FILE__": pa_simple_new() failed: %s\n", pa_strerror(error));
        if (s)
            pa_simple_free(s);
            return NULL;
    }
    while(!thread_exit_audio_send) {
        uint8_t buf[BUFSIZE];
        ...
        if (loop_write(socket_send_fd, buf, sizeof(buf)) != sizeof(buf)) {
